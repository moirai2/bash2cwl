...
sub test{
  ...
  #https://www.commonwl.org/user_guide/11-records/index.html (still working)
  ...
  #Multiple $commands
  my @commands=decoder("echo 'Hello World'|grep -v goodbye|wc -c>wcount.txt");
  tester2(\@commands,[{"command"=>"echo","input"=>["Hello World"],"stdout"=>"\$stdout1"},{"command"=>"grep","input"=>["-v","goodbye"],"stdin"=>"\$stdout1","stdout"=>"\$stdout2"},{"command"=>"wc","input"=>["-c"],"stdin"=>"\$stdout2","stdout"=>"wcount.txt"}]);
  $files=workflow(\@commands,undef,"wcount.txt");#116
  tester2($files->{"cwl/step1.cwl"},["cwlVersion: v1.0","class: CommandLineTool","baseCommand: echo","inputs:","  input1:","    type: string","    inputBinding:","      position: 1","  input2:","    type: string","outputs:","  output1:","    type: stdout","stdout: \$(inputs.input2)"]);#117
  tester2($files->{"cwl/step2.cwl"},["cwlVersion: v1.0","class: CommandLineTool","baseCommand: grep","inputs:","  input1:","    type: string","    inputBinding:","      position: 1","      prefix: -v","  input2:","    type: File","    streamable: true","  input3:","    type: string","outputs:","  output1:","    type: stdout","stdin: \$(inputs.input2.path)","stdout: \$(inputs.input3)"]);#118
  tester2($files->{"cwl/step3.cwl"},["cwlVersion: v1.0","class: CommandLineTool","baseCommand: wc","inputs:","  input1:","    type: boolean","    inputBinding:","      position: 1","      prefix: -c","  input2:","    type: File","    streamable: true","  input3:","    type: string","outputs:","  output1:","    type: stdout","stdin: \$(inputs.input2.path)","stdout: \$(inputs.input3)"]);#119
  tester2($files->{"workflow.cwl"},["cwlVersion: v1.0","class: Workflow","inputs:","  param1:","    type: string","  param2:","    type: string","  param3:","    type: string","  param4:","    type: string","  param5:","    type: boolean","  param6:","    type: string","outputs:","  result1:","    type: File","    outputSource: step3/output1","steps:","  step1:","    run: cwl/step1.cwl","    in:","      input1: param1","      input2: param2","    out: [output1]","  step2:","    run: cwl/step2.cwl","    in:","      input1: param3","      input2: step1/output1","      input3: param4","    out: [output1]","  step3:","    run: cwl/step3.cwl","    in:","      input1: param5","      input2: step2/output1","      input3: param6","    out: [output1]"]);#120
  tester2($files->{"workflow.yml"},{"param1"=>{"type"=>"string","value"=>"Hello World"},"param2"=>{"type"=>"string","value"=>"\$stdout1"},"param3"=>{"type"=>"string","value"=>"goodbye"},"param4"=>{"type"=>"string","value"=>"\$stdout2"},"param5"=>{"type"=>"boolean","value"=>"true"},"param6"=>{"type"=>"string","value"=>"wcount.txt"}});#121
  #variable
  @commands=decoder("output='Hello World'","echo \$output>output.txt");
  tester2(\@commands,[{"command"=>"_assign_","name"=>"output","value"=>"Hello World"},{"command"=>"echo","input"=>["\$output"],"stdout"=>"output.txt"}]);#122
  $files=workflow(\@commands,null,"output.txt");
  tester2($files->{"workflow.cwl"},["cwlVersion: v1.0","class: Workflow","inputs:","  param1:","    type: string","  param2:","    type: string","outputs:","  result1:","    type: File","    outputSource: step1/output1","steps:","  step1:","    run: cwl/step1.cwl","    in:","      input1: param1","      input2: param2","    out: [output1]"]);#123
  tester2($files->{"cwl/step1.cwl"},["cwlVersion: v1.0","class: CommandLineTool","baseCommand: echo","inputs:","  input1:","    type: string","    inputBinding:","      position: 1","  input2:","    type: string","outputs:","  output1:","    type: stdout","stdout: \$(inputs.input2)"]);#124
  tester2($files->{"workflow.yml"},{"param1"=>{"type"=>"string","value"=>"Hello World"},"param2"=>{"type"=>"string","value"=>"output.txt"}});
  @commands=decoder("output='Hello World'","string=Akira","echo \$output|grep -v \$string|wc>output.txt");#125
  tester2(\@commands,[{"command"=>"_assign_","name"=>"output","value"=>"Hello World"},{"command"=>"_assign_","name"=>"string","value"=>"Akira"},{"command"=>"echo","input"=>["\$output"],"stdout"=>"\$stdout1"},{"command"=>"grep","input"=>["-v","\$string"],"stdin"=>"\$stdout1","stdout"=>"\$stdout2"},{"command"=>"wc","stdin"=>"\$stdout2","stdout"=>"output.txt"}]);#126
  $files=workflow(\@commands,null,"output.txt");
  tester2($files->{"cwl/step1.cwl"},["cwlVersion: v1.0","class: CommandLineTool","baseCommand: echo","inputs:","  input1:","    type: string","    inputBinding:","      position: 1","  input2:","    type: string","outputs:","  output1:","    type: stdout","stdout: \$(inputs.input2)"]);#127
  tester2($files->{"cwl/step2.cwl"},["cwlVersion: v1.0","class: CommandLineTool","baseCommand: grep","inputs:","  input1:","    type: string","    inputBinding:","      position: 1","      prefix: -v","  input2:","    type: File","    streamable: true","  input3:","    type: string","outputs:","  output1:","    type: stdout","stdin: \$(inputs.input2.path)","stdout: \$(inputs.input3)"]);#128
  tester2($files->{"cwl/step3.cwl"},["cwlVersion: v1.0","class: CommandLineTool","baseCommand: wc","inputs:","  input1:","    type: File","    streamable: true","  input2:","    type: string","outputs:","  output1:","    type: stdout","stdin: \$(inputs.input1.path)","stdout: \$(inputs.input2)"]);#129
  tester2($files->{"workflow.cwl"},["cwlVersion: v1.0","class: Workflow","inputs:","  param1:","    type: string","  param2:","    type: string","  param3:","    type: string","  param4:","    type: string","  param5:","    type: string","outputs:","  result1:","    type: File","    outputSource: step3/output1","steps:","  step1:","    run: cwl/step1.cwl","    in:","      input1: param1","      input2: param3","    out: [output1]","  step2:","    run: cwl/step2.cwl","    in:","      input1: param2","      input2: step1/output1","      input3: param4","    out: [output1]","  step3:","    run: cwl/step3.cwl","    in:","      input1: step2/output1","      input2: param5","    out: [output1]"]);#130
  tester2($files->{"workflow.yml"},{"param1"=>{"type"=>"string","value"=>"Hello World"},"param2"=>{"type"=>"string","value"=>"Akira"},"param3"=>{"type"=>"string","value"=>"\$stdout1"},"param4"=>{"type"=>"string","value"=>"\$stdout2"},"param5"=>{"type"=>"string","value"=>"output.txt"}});#131
...
sub workflow{
  ...
  foreach my $command(@{$commands}){
    ...
    if($command->{"command"}eq"_assign_"){
      ...
      my $name="param".scalar(keys(%{$parameters})+1);
      $variables->{"\$".$command->{"name"}}=$name;
      push(@inputLines,"  $name:");
      push(@inputLines,"    type: string");
      $parameters->{$name}={"type"=>"string","value"=>$value};
      next;
    }
  ...
  foreach my $string(@strings){
    ...
    while($index<scalar(@chars)){
      ...
      if(exists($command->{"stdout"})&&$command->{"stdout"}eq"\|"){
        $piped="\$stdout$commandindex";
        $command->{"stdout"}=$piped;
      }else{
        $piped=undef;
      }
      if($command->{"command"}!~/^_/){$commandindex++;}
...
sub decodeVariable{
  ...
  for(;$index<scalar(@{$chars});$index++){
    if($chars->[$index]=~/[a-zA-Z_0-9]/){$value.=$chars->[$index];}
    else{last;}
  }
  return ("\$$value",$index);
}
